...
readonly MYSQL_DATABASE="pawbridge_user"
readonly MYSQL_USERNAME="pawbridge_user_app"
readonly KV_MOUNT="secret"
readonly KV_PATH="pawbridge/dev/user/mysql"
...
readonly POLICY_NAME="user-mysql-auth-read"
readonly ROLE_NAME="user-mysql-auth-read"
readonly BOUND_SERVICE_ACCOUNT="user-mysql-vault-auth"
...
readonly SCRIPT_DIR
readonly POLICY_FILE="${SCRIPT_DIR}/policies/user-mysql-auth-read.hcl"
...
usage() {
  cat <<'USAGE'
Usage:
  configure-user-mysql-auth-vso.sh check
  configure-user-mysql-auth-vso.sh apply
...
repairs the dedicated MySQL account so it can access only pawbridge_user.
...
on_error() {
  local exit_code=$?
  echo "User MySQL Vault bootstrap failed at line ${BASH_LINENO[0]}" >&2
  return "${exit_code}"
}
...
cleanup_token_session() {
  ...
  if [[ -n "${TOKEN_SESSION_DIR}" ]]; then
    case "${TOKEN_SESSION_DIR}" in
      /tmp/pawbridge-user-mysql-bootstrap.*) ;;
      *)
    ...
    if [[ "${cleanup_failed}" == false ]]; then
      if ! kube -n "${VAULT_NAMESPACE}" exec "${VAULT_POD}" -- \
        sh -c 'case "$1" in /tmp/pawbridge-user-mysql-bootstrap.*) rm -rf -- "$1" ;; *) exit 64 ;; esac' \
...
validate_local_inputs() {
  ...
  grep -Fqx 'path "secret/data/pawbridge/dev/user/mysql" {' "${POLICY_FILE}" || fail "missing exact KV data path"
  grep -Fqx 'path "secret/metadata/pawbridge/dev/user/mysql" {' "${POLICY_FILE}" || fail "missing exact KV metadata path"
  [[ "$(grep -Fc 'capabilities = ["read"]' "${POLICY_FILE}")" -eq 2 ]] || fail "policy must grant read only"
  if grep -Fq '*' "${POLICY_FILE}"; then
    fail "wildcards are not allowed in the User MySQL policy"
  fi
}
...
validate_token_session() {
  ...
  TOKEN_SESSION_DIR="$(kube -n "${VAULT_NAMESPACE}" exec "${VAULT_POD}" -- \
    mktemp -d /tmp/pawbridge-user-mysql-bootstrap.XXXXXX)"
  [[ "${TOKEN_SESSION_DIR}" == /tmp/pawbridge-user-mysql-bootstrap.* ]] || \
...
read_vault_credential() {
  ...
  [[ "${MYSQL_APP_PASSWORD}" =~ ^[0-9a-f]{64}$ ]] || \
    fail "Vault User MySQL password does not match the generated credential contract"
}

ensure_vault_credential() {
  if vault_credential_exists; then
    echo "Vault User MySQL credential metadata exists; the current value will be reused."
  else
    [[ "${MODE}" == apply ]] || fail "Vault User MySQL credential is missing"
    MYSQL_APP_PASSWORD="$(openssl rand -hex 32)"
    [[ "${MYSQL_APP_PASSWORD}" =~ ^[0-9a-f]{64}$ ]] || \
      fail "failed to generate the User MySQL password"
    write_new_vault_credential
    vault_credential_exists || fail "Vault User MySQL credential metadata is missing after write"
    echo "Created the User MySQL credential in Vault without printing its value."
  fi
...
validate_mysql_grants() {
  ...
  grants="$(mysql_exec root "${MYSQL_ROOT_PASSWORD}" mysql \
    ...
    fail "dedicated User MySQL account is missing or unreadable"
  ...
  while IFS= read -r grant_line; do
    case "${grant_line}" in
      ...
      "")
        ;;
      *)
        fail "dedicated User MySQL account has an unexpected privilege"
        ;;
    esac
  done <<< "${grants}"

  [[ "${database_grant_found}" == true ]] || \
    fail "dedicated User MySQL account lacks the exact database-scoped grant"
}
...
validate_mysql_login() {
  ...
  result="$(mysql_exec "${MYSQL_USERNAME}" "${MYSQL_APP_PASSWORD}" \
    ...
    fail "dedicated User MySQL login failed"
  [[ "${result}" == "1" ]] || fail "dedicated User MySQL validation returned an unexpected result"
}
...
GRANT ALL PRIVILEGES ON \`${MYSQL_DATABASE}\`.* TO '${MYSQL_USERNAME}'@'%';"
    if ! mysql_exec root "${MYSQL_ROOT_PASSWORD}" mysql "${sql}" >/dev/null 2>&1; then
      fail "failed to create or repair the dedicated User MySQL account"
    fi
    echo "Created or repaired the database-scoped User MySQL account."
  fi
  ...
  MYSQL_ROOT_PASSWORD=""
  MYSQL_APP_PASSWORD=""
  echo "Verified the User MySQL account grant and login."
}
...
main() {
  ...
  echo "User MySQL Vault bootstrap ${MODE} completed without printing secret values."
}
