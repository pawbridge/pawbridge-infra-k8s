...
readonly MYSQL_DATABASE="pawbridge_community"
readonly MYSQL_USERNAME="pawbridge_community_app"
readonly KV_MOUNT="secret"
readonly KV_PATH="pawbridge/dev/community/mysql"
...
readonly POLICY_NAME="community-mysql-auth-read"
readonly ROLE_NAME="community-mysql-auth-read"
readonly BOUND_SERVICE_ACCOUNT="community-mysql-vault-auth"
...
readonly SCRIPT_DIR
readonly POLICY_FILE="${SCRIPT_DIR}/policies/community-mysql-auth-read.hcl"
...
usage() {
  cat <<'USAGE'
Usage:
  configure-community-mysql-auth-vso.sh check
  configure-community-mysql-auth-vso.sh apply
...
repairs the dedicated MySQL account so it can access only pawbridge_community.
...
on_error() {
  local exit_code=$?
  echo "Community MySQL Vault bootstrap failed at line ${BASH_LINENO[0]}" >&2
  return "${exit_code}"
}
...
cleanup_token_session() {
  ...
  if [[ -n "${TOKEN_SESSION_DIR}" ]]; then
    case "${TOKEN_SESSION_DIR}" in
      /tmp/pawbridge-community-mysql-bootstrap.*) ;;
      *)
    ...
    if [[ "${cleanup_failed}" == false ]]; then
      if ! kube -n "${VAULT_NAMESPACE}" exec "${VAULT_POD}" -- \
        sh -c 'case "$1" in /tmp/pawbridge-community-mysql-bootstrap.*) rm -rf -- "$1" ;; *) exit 64 ;; esac' \
...
validate_local_inputs() {
  ...
  grep -Fqx 'path "secret/data/pawbridge/dev/community/mysql" {' "${POLICY_FILE}" || fail "missing exact KV data path"
  grep -Fqx 'path "secret/metadata/pawbridge/dev/community/mysql" {' "${POLICY_FILE}" || fail "missing exact KV metadata path"
  [[ "$(grep -Fc 'capabilities = ["read"]' "${POLICY_FILE}")" -eq 2 ]] || fail "policy must grant read only"
  if grep -Fq '*' "${POLICY_FILE}"; then
    fail "wildcards are not allowed in the Community MySQL policy"
  fi
}
...
validate_token_session() {
  ...
  TOKEN_SESSION_DIR="$(kube -n "${VAULT_NAMESPACE}" exec "${VAULT_POD}" -- \
    mktemp -d /tmp/pawbridge-community-mysql-bootstrap.XXXXXX)"
  [[ "${TOKEN_SESSION_DIR}" == /tmp/pawbridge-community-mysql-bootstrap.* ]] || \
...
read_vault_credential() {
  ...
  [[ "${MYSQL_APP_PASSWORD}" =~ ^[0-9a-f]{64}$ ]] || \
    fail "Vault Community MySQL password does not match the generated credential contract"
}

ensure_vault_credential() {
  if vault_credential_exists; then
    echo "Vault Community MySQL credential metadata exists; the current value will be reused."
  else
    [[ "${MODE}" == apply ]] || fail "Vault Community MySQL credential is missing"
    MYSQL_APP_PASSWORD="$(openssl rand -hex 32)"
    [[ "${MYSQL_APP_PASSWORD}" =~ ^[0-9a-f]{64}$ ]] || \
      fail "failed to generate the Community MySQL password"
    write_new_vault_credential
    vault_credential_exists || fail "Vault Community MySQL credential metadata is missing after write"
    echo "Created the Community MySQL credential in Vault without printing its value."
  fi
...
validate_mysql_grants() {
  ...
  grants="$(mysql_exec root "${MYSQL_ROOT_PASSWORD}" mysql \
    ...
    fail "dedicated Community MySQL account is missing or unreadable"
  ...
  while IFS= read -r grant_line; do
    case "${grant_line}" in
      ...
      "")
        ;;
      *)
        fail "dedicated Community MySQL account has an unexpected privilege"
        ;;
    esac
  done <<< "${grants}"

  [[ "${database_grant_found}" == true ]] || \
    fail "dedicated Community MySQL account lacks the exact database-scoped grant"
}
...
validate_mysql_login() {
  ...
  result="$(mysql_exec "${MYSQL_USERNAME}" "${MYSQL_APP_PASSWORD}" \
    ...
    fail "dedicated Community MySQL login failed"
  [[ "${result}" == "1" ]] || fail "dedicated Community MySQL validation returned an unexpected result"
}
...
GRANT ALL PRIVILEGES ON \`${MYSQL_DATABASE}\`.* TO '${MYSQL_USERNAME}'@'%';"
    if ! mysql_exec root "${MYSQL_ROOT_PASSWORD}" mysql "${sql}" >/dev/null 2>&1; then
      fail "failed to create or repair the dedicated Community MySQL account"
    fi
    echo "Created or repaired the database-scoped Community MySQL account."
  fi
  ...
  MYSQL_ROOT_PASSWORD=""
  MYSQL_APP_PASSWORD=""
  echo "Verified the Community MySQL account grant and login."
}
...
main() {
  ...
  echo "Community MySQL Vault bootstrap ${MODE} completed without printing secret values."
}
