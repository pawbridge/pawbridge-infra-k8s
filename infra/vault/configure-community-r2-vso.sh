...
readonly POLICY_NAME="community-r2-read"
readonly ROLE_NAME="community-r2-read"
readonly BOUND_SERVICE_ACCOUNT="community-r2-vault-auth"
...
readonly POLICY_FILE="${SCRIPT_DIR}/policies/community-r2-read.hcl"
...
usage() {
  cat <<'USAGE'
Usage:
  configure-community-r2-vso.sh check
  configure-community-r2-vso.sh apply
...
on_error() {
  local exit_code=$?
  echo "Community R2 Vault bootstrap failed at line ${BASH_LINENO[0]}" >&2
  return "${exit_code}"
}
...
cleanup_token_session() {
  ...
  if [[ -n "${TOKEN_SESSION_DIR}" ]]; then
    case "${TOKEN_SESSION_DIR}" in
      /tmp/pawbridge-community-r2-bootstrap.*) ;;
      *)
    ...
    if [[ "${cleanup_failed}" == false ]]; then
      if ! kube -n "${VAULT_NAMESPACE}" exec "${VAULT_POD}" -- \
        sh -c 'case "$1" in /tmp/pawbridge-community-r2-bootstrap.*) rm -rf -- "$1" ;; *) exit 64 ;; esac' \
...
validate_local_inputs() {
  ...
  if grep -Fq '*' "${POLICY_FILE}"; then
    fail "wildcards are not allowed in the Community R2 policy"
  fi
}
...
validate_token_session() {
  ...
  TOKEN_SESSION_DIR="$(kube -n "${VAULT_NAMESPACE}" exec "${VAULT_POD}" -- \
    mktemp -d /tmp/pawbridge-community-r2-bootstrap.XXXXXX)"
  [[ "${TOKEN_SESSION_DIR}" == /tmp/pawbridge-community-r2-bootstrap.* ]] || \
...
main() {
  ...
  echo "Community R2 Vault bootstrap ${MODE} completed. No R2 values were read or written."
}
